...
| Standing | Milestone count | DP earned via Advancement | Typical Attribute+Skill points* | Rough shape of the character |
|---|---|---|---|---|
| **Green** | Milestone 0 | 0 DP | 12 | Fresh off character creation. Everything on the sheet came from the Novice Hero build. |
| **Blooded** | Milestone 1–2 | 3–6 DP | 12–13 | Survived the early sessions. Usually one Skill bump or a second Tier 1 Feat — not yet an Attribute increase. |
| **Veteran** | Milestone 3–5 | 9–15 DP | 13–15 | First Attribute increase has usually landed by now. Tier 2 Feats start becoming affordable as prerequisites catch up. |
| **Hardened** | Milestone 6–9 | 18–27 DP | 16–17 | Multiple Attribute increases banked. Wound Threshold and/or Stress Limit have visibly grown past the creation baseline. |
| **Storied** | Milestone 10+ | 30+ DP | 18+ (open-ended) | Late-campaign. Pushing into Tier 3 Feats, most relevant Attributes and Skills sitting at or near the +3 mortal cap. |

*\*"Attribute+Skill points" is the sum of every point on the sheet — all four Attributes plus every Skill rank — the same number the Bestiary's enemy budgets are measured in (see "Enemy Budget by Party Standing" in Beasts, Monsters, Mutants). Green's 12 is a hard number, checked against six independently-built creation-day characters (Helga, Faelan, Pillit, and Ox/Morwenna/Perpetua before their first Milestone), all of whom land on exactly 12 — the 4-DP Attribute budget and the Link Rule's cost curve converge on that total almost regardless of build shape. Everything past Green is a working average from three Hardened characters (Ox, Morwenna, Perpetua — 18, 21, and 24 DP earned respectively, all landing at 16 Attribute+Skill points), extrapolated linearly for Blooded/Veteran/Storied. It holds only for a "balanced" spender mixing Attribute/Skill purchases with Feats and Spells — Feats and spell-learning cost DP but add nothing to this total, so a character who spends every Milestone on Feats stays at their Green number regardless of Standing (Wren is the roster's live example: Milestone 1, but her one Milestone went entirely to Parasitic Momentum, so her Attribute+Skill total never moved off her creation-day number). Treat this column as a GM-planning average, not a guarantee about any specific character's sheet.*

A character's Standing is a snapshot of Milestone count only — it says nothing about *how* those points were spent, and two characters at the same Standing can look completely different at the table (see the Focus vs. Spread design note in The Meat). Use Milestone count for anything that needs to be exact; use Standing for the two-second version.

*(Note: this naming and these band boundaries are a first pass, not settled canon — worth retuning once a few characters have actually been played through Hardened/Storied territory to see whether the cutoffs feel right in practice.)*