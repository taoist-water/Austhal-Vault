# Coin and Clutter: Mundane Goods & Services

_A conversion of the classic "Goods and Services" and "Armor and Shields" tables into Iron & Marrow terms._

## Conversion Philosophy (Read This First)

This is **not** a gp-to-sp currency conversion. If it were, a suit of Full Plate would cost 1,500 sp instead of the 200 sp already printed in _Hardware_, and every price in this document would contradict the core book on sight. Instead, every item below is priced by **analogy** — anchored against what's already established:

- **Currency:** sp is the base unit. cp for anything trivial (10 cp = 1 sp). gs only for the handful of items that are genuinely nobility/cartel-tier purchases (per the existing "gold is held only by nobility and wealthy cartels" fluff).
- **Availability bands = price bands.** I'm using the bands _Soothing the Soul_ already defined for Acquisition, since they're the only official price logic in the system: **Common 1–25 sp, Scarce 15–50 sp, Rare 50–200+ sp, Legendary GM-set.** Everything below was slotted into these bands rather than mathematically derived.
- **Slots follow the existing sizing logic exactly** — 0 Slots for pocket clutter, 1 Slot for standard gear, 2 Slots for bulky/awkward gear (per _Hardware_'s Visual Slot System). I did not invent a new slot tier.
- **No new subsystems.** Where an item has no obvious mechanical hook, it stays flavor-only. Most of what follows is texture, not a lever the dice touch — that's consistent with the system's stated goal of reducing cognitive load.
- **Overlap check first.** Several d20 items already have a mechanically-identical twin in _Hardware_. Where that's true, I say so and don't create a duplicate entry.

One flagged item needs your sign-off before it's real: a proposed expansion of the core Armor table (Section 9). Everything else here is safe to treat as final.

---

## 0. Two Rules Clarified During This Exercise

Both of these resolve real ambiguity in the core system rather than just this equipment list — worth considering for _Hardware_'s Inventory section proper, even though I'm formalizing them here for now.

### 0a. What the Community Supply Die Actually Tracks

_Hardware_ abstracts the party's shared consumables into the Die so nobody tracks individual arrows, torches, or waterskins. Concretely, that's:
...
**The dividing line for anything new that comes up later:** if it's _used up in the doing_ — an arrow loosed, oil burnt, a ration eaten, a bandage wound around a cut — it's the Die's problem. If it _still exists and still works_ after you use it — a bow, a lantern, a crowbar, a coil of rope — it's a normal Slotted item, tracked individually as in Section 1.
...
### 0b. Containers Don't Multiply Slots

A container's listed Slot cost is for **the object itself**, full stop. Owning one never grants extra carrying capacity — it isn't a nested storage space, it's an item like any other. If it worked that way, one 2-Slot chest would let a character haul unlimited gear for a single purchase, and the Attrition Tax (Wounds forcing you to drop gear) evaporates the moment anyone thinks to buy a box.
...
**On _Hardware_'s existing "small treasure chest" example (2 Slots):** that's a _found-loot_ abstraction — chest-plus-contents collapsed into one carry-out object hauled from a dungeon, not an empty container a character buys and refills over a campaign. Same underlying rule (rigid object, fixed Slot cost), different use case, so it doesn't actually contradict anything — it just wasn't spelled out until now.

**The Backpack** (Section 1) is still 0 Slots and worn-exempt, but to be explicit: it doesn't add capacity either. It's the fictional wrapper for the Pack designation, not a bag of holding — the Pack's actual size is fixed entirely by the character's Prowess-derived Slot total, independent of what container physically holds it.
...
## 2. Special Substances & Alchemical Items

Several d20 items already have a mechanical twin in _Hardware_'s Alchemical Wares table. I'm flagging the overlaps rather than duplicating, and only writing new entries for genuine gaps.

|d20 Item|Iron & Marrow Treatment|
|---|---|
|Acid (flask)|Reskin of **Vitriol Solvent** (25 sp, Rare) — no new item.|
|Alchemist's fire (flask)|Reskin of **Naphtha Fire-Flask** (30 sp, Rare) — no new item.|
|Sunrod|Cheap, single-scene cousin of **Everburning Torch** below — see new entry.|
|Tindertwig|Folded into the Supply Die abstraction (see Section 1).|
...
**A seam worth flagging while I built this table:** _Manipulating the Void_ uses **Novice / Adept / Master** for Arcana spell levels, while _Embracing the Abyss_'s Domain of Law Miracles use **Novice / Apprentice / Master**. Same three tiers, different word for the middle one across two documents. Not urgent, but worth a pass to pick one term for "tier 2" and use it everywhere — right now a GM reading both books back to back has to mentally reconcile "Adept" and "Apprentice" as the same rung.

---

## 9. Armor & Shields — Mapping Table 6-6

This is the section that actually touches core mechanics, so I've kept it separate and conservative. Iron & Marrow deliberately runs four armor tiers instead of d20's ten — that's a design choice, not an oversight, so my default move below is **reskin**, not **invent**.

|d20 Armor|Iron & Marrow Equivalent|Treatment|
|---|---|---|
|Padded|Padded (+0, Cushioned)|Direct match|
|Leather|Leather (+1)|Direct match|
|Studded Leather|Leather (+1)|Reskin — fancier stitching, same niche, no new tier needed|
|Hide|Chainmail/Scale (+2, Bulky)|Reskin — "primitive" flavor variant|
|Scale Mail|Chainmail/Scale (+2, Bulky)|Direct match (literally the same name already)|
|Chainmail|Chainmail/Scale (+2, Bulky)|Direct match|
|Splint Mail / Banded Mail|Plate Armor (+4, Restricted)|Reskin|
|Full Plate|Plate Armor (+4, Restricted)|Direct match, already priced at 200 sp|
|Buckler|Buckler (2 SV, Nimble)|Direct match|
|Light/Heavy Wooden or Steel Shield|Kite/Round Shield (4 SV, Cover)|Reskin|
|Tower Shield|Tower Shield (5 SV, Bulwark, Obstructive)|Direct match|
|Armor spikes|New minor add-on|See below|
|Gauntlet, locked|New minor add-on|See below|
|Shield spikes|New minor add-on|See below|

**Everything above needs no sign-off — it's pure reskinning of what already exists.** Two d20 entries don't have a clean home, though:

### [DESIGNER SIGN-OFF NEEDED] Two Proposed New Armor Entries

- **Chain Shirt** — d20's "light but still +4 AC" niche has no I&M equivalent. Proposed: **+2 Armor, Light (no Bulky tag), 70 sp, Scarce.** Same protection as Chainmail/Scale, priced higher specifically because it _lacks_ the Bulky penalty — you're paying for mobility, same logic Masterwork Quality already uses to price out negative-tag suppression.
- **Breastplate** — d20 carves out a "heavy medium" niche between Chainmail and full Plate. Proposed: **+3 Armor, Medium, Bulky, 120 sp, Rare.** Also absorbs Half-Plate as a reskin.

If approved, these become two new rows in _Hardware_'s core Armor table (currently 4 entries → 6). I haven't touched that file — flagging it here for your call rather than silently expanding the core table.

**New minor add-ons (low-risk, not core-table changes):**

|Item|Cost|Availability|Effect|
|---|---|---|---|
|Armor spikes|15 sp|Scarce|When an enemy loses a Grab or Shove Clash against you, they suffer 1 Impact from the spikes.|
|Gauntlet, locked|5 sp|Common|Advantage on Prowess checks to resist being disarmed.|
|Shield spikes|8 sp|Common|Shove actions made with this shield deal +1 Impact on a win.|

**On Max Dex Bonus / Armor Check Penalty / Arcane Spell Failure:** these don't need conversion at all. I&M already expresses all three concepts through the Bulky and Restricted tags on the existing table — Bulky already imposes -1 to Athletics, Stealth, _and_ Arcana (i.e., ASF is already covered), and Restricted already kills Arcane casting outright and cuts Movement by 10 ft. Reintroducing a separate numeric penalty stack on top of that would just recreate the exact granular bookkeeping the system was built to avoid. Leave it alone.

---

## 10. Open Items Requiring Your Sign-Off

1. **Chain Shirt (+2 Light) and Breastplate (+3 Medium, Bulky)** — do you want these folded into _Hardware_'s core Armor table? Everything else in this document is safe to treat as final regardless of your answer here.
2. **Novice/Adept/Master vs. Novice/Apprentice/Master** terminology drift between _Manipulating the Void_ and _Embracing the Abyss_ — worth a single unifying pass, not urgent.
3. **Thieves' Tools** was missing from _Hardware_ entirely despite Thievery being a listed skill — I've added it in Section 3 as a low-risk gap fill, not a structural change, so no sign-off needed there, just flagging that it existed as a hole.
4. **Section 0 (Supply Die scope + container/Slot rule)** resolves ambiguity that lives in the _core_ Inventory rules, not just this equipment list. Low risk to adopt outright, but it's a candidate for being moved into _Hardware_ proper rather than left parked in a goods-and-services supplement — your call on where it should live long-term.