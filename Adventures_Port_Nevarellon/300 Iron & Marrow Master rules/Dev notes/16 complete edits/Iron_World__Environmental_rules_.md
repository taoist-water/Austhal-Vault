...
- *The Mechanic:* When facing severe conditions (a blizzard, a scorching desert, freezing water), the GM calls for a Hazard Check—usually 2d6 + Wits + Survival to navigate it safely, or Prowess + Athletics to physically endure it against TN 8.
...
**Determining Awareness:** Compare the character's Passive Notice (see Iron Core) against the hazard's Concealment Rating using the unopposed Passive Notice mode — Passive Notice + Situational Modifier vs. TN8. Clearing the TN makes the character Aware; failing it leaves them Unaware. A GM can still call for an active Notice check instead when a player specifically declares they're searching (checking a chest for a needle before opening it, sweeping a corridor for tripwires) — Passive Notice governs the default, unprompted case only.
