...
| Wand                                  | Arcane Focus         | —     | 1H    | —                               | Conduit, Warded Core, Sidearm.                                                                                                                                                              | 20 sp | Scarce           |
...
- Warded Core: When the wielder rolls Snake Eyes on an Arcana check while holding this item, they may destroy it (it immediately gains the Ruined condition) to treat the result as a standard Failure instead — no physical Wound, no Dissonant Stress from the fumble, and no battlefield hazard. A Ruined item with this tag can be restored via Hammer & Forge downtime, same as any other Ruined item. (Renamed from the old "Focus" tag to avoid collision with the Philter of Focus alchemical item — same rename, no other change to how any existing spell or feat references it, since nothing else in the system referenced "Focus" the tag by name.)
